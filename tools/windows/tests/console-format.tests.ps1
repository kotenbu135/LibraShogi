...
Check "自動計測（実行中）" (Strip (Format-Auto-Status ([pscustomobject]@{ enabled = $true; every_games = 400000 }) ([pscustomobject]@{ running = [pscustomobject]@{ kind = "match"; started = $now } }) 2000168 1)) `
    "match 実行中（<いつ>）"
