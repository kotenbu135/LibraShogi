...
- 終局規定は docs/rules.md（大会規定: 千日手4回＝引き分け、連続王手は王手側負け、入玉宣言法27点、256手引き分け）が唯一の正。マッチの裁定はハーネスが行う。
...
- ユーザーに確認が要る事項: 256手の数え方、対局棋譜の公開可否、vast.ai の利用開始、desktop への終局判定追加の時期。
