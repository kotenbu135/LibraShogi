...
- 2026-09-15 に ls の step 477,636 を v0.1 とし、GitHub で公開した（重みは `~/libra-run/releases/v0.1/`、ユーザーの決定）。公開リポジトリなので、コミットに秘密情報・ホストの IP・私的な情報を入れない。文書・コメント・コミットメッセージに個人名（ユーザーの表示名など）を書かず、「ユーザー」と書く（2026-09-24 のユーザーの依頼）。コミットのメールはリポジトリの設定の GitHub noreply。
...
5. **ユーザーに聞く事項**（勝手に決めない）: 対局棋譜（水匠5 との）の公開可否、vast.ai の利用開始、公開版サイトの規定を大会規定に揃えるか、desktop への終局判定追加の Issue を出す時期（Libra 側の準備ができたら起票する決定済み）、基準値マッチの持ち時間、大きな設計変更（ネットの形、学習則、ルール解釈）、ユーザーが決めた条件（起動の条件、学習データの変更は 1 日 1 つ、計測の手順など）を緩める・例外にすること（緩めた版で進めず、何をどう緩めるかを先に聞く。2026-09-17 の起動の条件「記録なしなら起動しない」を Claude が「煙テストまで仮」に緩め、伝えないまま 8 日動かしたため）。それ以外は自分で決めて decisions.md に記録する。
...
## ユーザーへの説明（将棋 AI の知識を前提にしない）
ユーザーはルール設計者で、将棋 AI・機械学習の専門家ではない（2026-09-18 のユーザーの依頼「知識がなくてもわかりやすく理解できるように」）。
- **結論を最初の 1〜2 文で**（「問題ありません」「合格です」「まだ始まっていません」）。次に理由、最後に次の予定や頼みたい操作。
- **専門用語は初めて出すときに一言で言い換える**（下の表）。略語・内部の名前（M1〜M4、変数名、ファイル名）だけで書かない。
- **物差しや設定は「何を確かめるためのものか」から説明する**（例:「最強比は、これまでで最も強かった自分に勝てるかを見る。伸びが止まっていないかが分かる」）。
- **数値には読み方を添える**（Elo の差と勝率の目安、区間の下限が 0 より上なら偶然ではない、など）。比べる相手（前回の値・基準）も並べる。
- **「なぜ今そうしているか」を理由で書き**、たとえが効くときは短いたとえを使う（「lx は本体の弱点ばかり探す専門の練習相手」）。
- **確かめていないことは「未確認」と書き**、見込みと実測を分ける。問題や自分の誤りは隠さず先に書く。
- ユーザーに操作を頼むときは、何を押すか・いつまでにかを 1 文で書く。
...
| リプレイの窓 | 学習に使う最近の対局の範囲。狭すぎると対局の結果を覚えるだけになる（2026-09-17 のやり直しの原因） |
| held-out・一般化 | 学習に使わずに取っておく対局。そこでも予測が当たれば「覚えただけでなく身についている」（一般化） |
| Elo | 強さの差を表す数。+100 で勝率約 64%、+200 で約 76%、+300 で約 85% |
| 最強比・基準比・固定の参照 | それぞれ、これまで最強の自分・少し前の自分・変わらない過去の重みとの対局で強さを測る |
| 区間（95%） | 局数が有限なことによる幅。下限が 0 より上なら、たまたまではなく強くなった |
| 煙テスト・本番 | 煙テストは設定が正しく伸びるかを短く確かめる最初の段階。合格したら本番として続ける |
| ワーカー・ホスト（vast.ai） | 自己対局だけを手伝う借りた GPU の計算機 |
...
- **ls・lx を監視しない**。バックグラウンドタスク・待機ループ・ポーリングで ls・lx の稼働状態や自動計測の終了を待たない。状態が要るときは `bin/libra [--run lx] status` を 1 回読むだけにして、待たずに作業を終える。結果は次にユーザーから聞かれたときに読む。**ls・lx 以外のジョブ（玉配置表の検証対局 `libra-scale seq`、vast.ai のセッションなど）は監視してよい**（2026-09-15 のユーザーの決定「ls・lx 以外は監視を許可」）。アラート・失敗・節目を Monitor で拾い、課金が続くものは特に起動と同時に置く。
  - 例外: **節目の自動定期確認**（2026-09-25 のユーザーの決定）。クラウドの定期実行が 3 時間おきに `progress` ブランチを 1 回読み、40 万局の節目の自己評価がそろったときだけ報告する。GitHub の記録を読むだけで、ランの操作・設定・`bin/libra` には触れず、待機ループも持たない。
