...
11. **学習データに関わる変更（探索・目標・リプレイ・設定値）は 1 日 1 つまでにし、前後で強さと一般化の物差し（docs/restart-plan.md §3: `gen` の held-out との差、最強比 1,000 局、固定の参照）を比べてから次を入れる。**「棋譜は同じ」「結果は同じ」は本番の大きさ（同じ seed・同じ設定の自己対局の記録の一致）で確かめてから書く。学習則の設定値は出所（一次資料）・理由・確かめる計測を docs/ls2-settings.md の表に書いてからでないと本番の run に入れない（「記録なし」のまま起動しない）。2026-09-17 に、9/11 の実装時に出所なしで決めた窓 10 万局・replay_ratio 4 が窓の記憶を起こし、5 日間の学習をやり直すことになったため（docs/status-2026-09-17.md §10、docs/restart-plan.md §1）。
