...
DEFAULT_THRESHOLDS = {
    ...
    # 一度この線を超えた run で下回ったら「見直し」（docs/restart-plan.md §3 M1）。
    # **この線は自己対局の分布に貼り付いている**: held-out の本将棋の目標はその局の結果 z そのものなので、
    # 相関の高さは「今の自己対局の結果がどれだけ読めるか」で決まる（上の docstring）。2026-09-17 に 0.6 と
    # 決めたときの水準は 0.65〜0.68 で、線はその約 0.08 下だった。2026-09-22 に投了を入れて結果が
    # 分かりきった終盤が棋譜から消えると水準が 0.56〜0.60 に下がり、同じ margin で 0.5 に引き直した
    # （2026-09-23。decisions.md。学習は健全で、布石の相関は落ちていないことを確かめてある）。
...
def _gen_rows(sd: StateDir, window_games: int) -> list[dict]:
    """gen の行（同じ gen を繰り返す行は 1 つにする）を、最新の局数から window_games 局ぶん。"""
    rows, seen = [], set()
    for r in load_metrics(sd, 100000):
        g = r.get("gen")
        if not g or not (g.get("heldout") or {}).get("normal"):
            continue
        key = (g.get("t"), g.get("games"))
        if key in seen:
            continue
        seen.add(key)
        rows.append(r)
