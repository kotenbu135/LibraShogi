...
def test_m1_flat_is_ok_and_fall_warns(tmp_path):
    ...
    # 一度 0.5 を超えた run が 0.5 を割った → 見直し（窓との差は 0.1 以内にして、丸暗記の判定と分ける）
    write(flat[:6] + [(0.52, 0.45), (0.51, 0.46), (0.52, 0.45), (0.50, 0.44), (0.51, 0.45), (0.52, 0.46)])
    r = review(sd, now=now)["items"][0]
    assert r["verdict"] == REVIEW and "下回った" in r["why"], r

    # まだ一度も 0.5 に届いていない run（学習の初め）は、0.5 未満でも見直しにしない
    write([(0.30, 0.28), (0.32, 0.30), (0.34, 0.31), (0.36, 0.33), (0.38, 0.35), (0.40, 0.37)])
    assert review(sd, now=now)["items"][0]["verdict"] == OK

    # 投了を入れたあとの実際の水準（窓 0.68・held-out 0.58、この run の最高 0.71）は見直しにしない。
    # 2026-09-23、480 万局。0.6 のままだと「見直し」が出続けるので 0.5 に引き直した（decisions.md）
    write([(0.68, 0.58), (0.68, 0.57), (0.69, 0.59), (0.68, 0.56), (0.68, 0.58), (0.68, 0.58),
           (0.68, 0.57), (0.68, 0.59), (0.68, 0.58), (0.68, 0.58), (0.68, 0.57), (0.68, 0.59)])
    r = review(sd, now=now)["items"][0]
    assert r["verdict"] == OK, r
    assert r["values"]["gap"] < 0.1  # 丸暗記の判定（窓との差）は別で、こちらは掛かっていない
