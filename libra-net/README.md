...
学習ループ（AdamW、bf16）と損失（方策は全読み局面だけ、疎な改善方策とのクロスエントロピー。価値は WDL の soft ラベル。補助 V̂41 は布石局面だけ）は `libra-league/libra_league/trainer.py`（`soft_ce`、`Trainer`）。損失の重みと価値目標の作り方は docs/method-evidence.md §1。蒸留は未実装。
