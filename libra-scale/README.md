...
- **アラート**: 対称な組が後手に傾いたら、log.txt の `ALERT` 行と `ALERT.txt` に出す。`--notify windows` を付けると Windows の通知領域にも出す（WSL の powershell.exe）。途中の判定は「先手の得点 + 3.0·se < 0.5」、止まった後の判定は 95% 区間の上限 < 0.5。
