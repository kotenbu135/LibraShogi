...
bin/libra-scale seq run --dir $D --table ~/libra-run/ls/scale/scale-v0.1.json   # 初回（V̂ とモデルは --table から）。再開は --dir だけ
...
- **アラート**: 対称な組が後手に傾いたら、log.txt の `ALERT` 行と `ALERT.txt` に出す。途中の判定は「先手の得点 + 3.0·se < 0.5」、止まった後の判定は 95% 区間の上限 < 0.5。
