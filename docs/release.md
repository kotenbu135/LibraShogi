...
|---|---|---|
| A | どの節目の重みを v0.2 にするか | **360 万局の節目の archive**（2026-09-21 のユーザーの決定で決まった。§0） |
| B | 玉配置表を全組で作り直すか（ls・lx を約 12 時間止める） | **全 492 組で作り直す**（2026-09-21 のユーザーの決定）。§2 |
| C | エンジンの版の名乗り（`LibraShogi 0.0.2`）を上げるか | **`LibraShogi 0.2.0` に上げる**（2026-09-21 のユーザーの決定。配布物 v0.2 と揃える）。実装済み |
| D | `Scale_Table` の既定（今は空。実行ファイルの隣の `scale.json` を自動で見ない） | 変えない（v0.1 と同じ挙動のまま） |
| E | 外部エンジンとの対局棋譜を公開するか | **出さない**（2026-09-21 のユーザーの決定。v0.1 と同じ） |
| F | 自己対局の棋譜をどこまで出すか | v0.1 と同じく前後 1 時間の標本だけ Release に載せる（未回答。既定で進める） |

**残るのは D だけ**（2026-09-21 時点）。A・B・C・E は上のとおり決まった。D は decisions.md 2026-09-16 で「今回は変えない、どちらを変えるかはユーザーの判断待ち」のまま残っている。
