...
| 布石の途中の表示（`JUDGE_SIMS` 1 ＝ネット単体） | 布石の途中の局面のネット単体は実際に近い（先手の番 +0.002、後手の番 +0.015） | 19:10 | ずれの大きさは未確認 |
...
| 案 | 所見 | 推奨 |
|---|---|---|
| 選ぶ AI の読みを増やす | サイトはすでに 128 回で、得点はいつも先手と同じ。エンジンは持ち時間で読む（`movetime 1000` で中央値 約 3,000 ノード、measurements 同日） | 変えない |
| 検証の結果から求めた補正値を V から引く | 釣り合う組でのずれは有意でない | 入れない |
| 布石の途中の表示 | ずれの大きさが未確認 | 直すなら先に測る |
...
**推奨**
- 急ぐ理由は弱い。釣り合う組での判断のずれは有意でなく、サイトの選ぶ AI の損もいつも先手と同じ水準（4.1）。
- 次の順で進める:
  1. 4.4 の (a) で、学習の目標と実際の結果の差を毎回見えるようにする。学習則は変えない。
  2. (1) を 1.0 にし、V̂41 ヘッドは残す案を、比べてから決める。
- (1) を 1.0 にする案をすすめる理由:
...
- lx も `lambda_z` 0.5 で同じ形なので、変えるなら lx も合わせるかを決める。基準比の Elo は、基準が固定のネットなので変えた後も続けて測れる。
