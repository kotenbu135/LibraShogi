...
**自動計測（`[auto]`、本体 ls のみ有効）**: `every_games`（総局数がその分たまるごと。0 なら `every_hours` の時間ごと。2026-09-17 から局数が既定の運用）にチェックポイントを `checkpoints/archive/` に残し、`libra eval`（`eval_sims`=96）で 2 通りの対局をする。
