...
釣り合う段が分からないうちは `match_go_opp` に複数書いて挟む。段をまたいで布石は同じなので、段どうしの比較が対になる。
`self` は最初の段が作った布石を `matches/auto-<時刻>.openings.jsonl` に控え、残りの段はそれを使う（`--opening-file`）。
エンジンは根の乱数を時刻から取り、読みも複数スレッドなので、種をそろえても布石はそろわないため（2026-09-26 まではそろっていなかった）。
`selfplay`・ファイルの形は `--fuseki-seed` をそろえれば同じ局面を選ぶ。年末の 100 局の基準値マッチは持ち時間をそろえたまま別に行う。
