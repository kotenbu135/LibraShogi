...
- **固定の参照（`reference_ckpts`・`reference_games`、同 M4）**: 設定に書いた重みのファイル（例: 旧 ls の 646,699 と実験の win1m.pt）と毎回対局し、`eval/reference.jsonl` に残す。run をまたいで同じ相手なので、絶対の物差しになる。コンソールの Elo タブに「対 <参照>」の線で出る。**参照に 80% 以上勝つようになったら、そのときの archive を参照に足す**（勝率が 1 に寄ると Elo が縮み、伸びていても曲線が寝て見える。docs/scaling-2026-09-18.md §6.4）。
