...
| 自己対局と学習を自分の機械で回したい（v0.1 の続きも） | §1〜§3 → §6 |
...
| モデルの入手 | 手順 |
|---|---|
| 公開版の重みを使う | [Releases](https://github.com/kotenbu135/LibraShogi/releases) の `libra-v0.1.onnx`（と玉配置表 `scale-v0.1.json`）を取得。中身は [model-card-v0.1.md](model-card-v0.1.md) |
...
```bash
export LIBRA_MODEL=~/Downloads/libra-v0.1.onnx
...
- 天秤将棋の 1〜2 手目（両玉）に表を使うには `setoption name Scale_Table value <scale-v0.1.json の場所>` を送る（既定は空で、表無しでも探索で置く）。
...
**Windows（ビルドしない）**: [Releases](https://github.com/kotenbu135/LibraShogi/releases) の `libra-v0.1-windows-x64.zip` を展開し、GUI の「エンジン」に `libra.exe` を登録する。
モデルは隣の `libra.onnx` を自動で読む。**`Scale_Table` オプションに、隣の `scale.json` の場所を入れる**（既定は空で、自動では見ない）。
...
**v0.1 の続きから学習する**: Releases の `libra-v0.1.pt` にはモデル・オプティマイザ・step・乱数状態・設定が入っている。
新しい run-id の `~/libra-run/<run-id>/checkpoints/latest.pt` に置いてから `bin/libra --run <run-id> run` すると、その step から続く（本体 ls もこの形で旧ルールの重みから始めた。[runbook.md](runbook.md) 冒頭）。`[net]` は v0.1 と同じにする。
...
| 玉配置表（両玉の釣り合う組） | Releases の `scale-v0.1.json`（CC0） | 形は [libra-scale/README.md](../libra-scale/README.md) |
| 自己対局の棋譜（標本） | Releases の `libra-v0.1-selfplay-sample.jsonl.gz`（CC0） | 形は [data/README.md](../data/README.md) |
...
bin/libra match --games 20 --go "movetime 3000" \
  ...
  --model ~/Downloads/libra-v0.1.onnx --libra-opt Scale_Table=$HOME/Downloads/scale-v0.1.json
```
...
配布物（zip、SHA256SUMS、自己対局の標本）は `tools/package_release.sh`（[model-card-v0.1.md](model-card-v0.1.md) §9）。CI の `windows-cross` ジョブが同じクロスビルドを毎回確かめる。
